...
# ofperiods where we need to compute forecasts.
...
for (i in 1:P) {
   ...
   AR2_DirectForecast_lm <- lm(GDPGrowth_estimating_sample ~ lag(GDPGrowth_estimating_sample,(i)) + lag(GDPGrowth_estimating_sample,(1+i)))   
   ...
   #     the same as T+1 in our usual notation. This means that to get the observation for
   #     period T, we use [FirstForecast_YearQuarter - (0.25)].
   direct_forecasts[i] <- (beta_0_hat + (beta_1_hat %*% GDPGrowth_xts[FirstForecast_YearQuarter - (0.25)]) 
                       + (beta_2_hat %*% GDPGrowth_xts[FirstForecast_YearQuarter - (0.50)]))  
}
