...
# We will now run the ADL test and see how it works:
...
Stationary_DickeyFuller_lm <- lm(diff(y_stationary_xts) ~ lag(y_stationary_xts,1))
...
UnitRootAR2_DickeyFuller_lm <- lm(diff(y_unit_root_AR2_xts) ~ lag(y_unit_root_AR2_xts,1) + lag(diff(y_unit_root_AR2_xts),1) +  lag(diff(y_unit_root_AR2_xts),2))
...
TrendStationaryAR2_NoTrend_lm <- lm(diff(y_AR2_TrendStationary_xts) ~ lag(y_AR2_TrendStationary_xts,1) + lag(diff(y_AR2_TrendStationary_xts),1) +  lag(diff(y_AR2_TrendStationary_xts),2))
...
chow_test_statistics <- array(c(0), dim = c(num_tests))
for (i in 1:num_tests) {
  D <- 1*(time(y_break50_xts) > time(y_break50_xts)[tau[i]])  
  Chow_test_model = lm(y_break50_xts ~ lag(y_break50_xts,1) + D + (D*lag(y_break50_xts,1)))
  coeftest(Chow_test_model)
  chow_test = linearHypothesis(Chow_test_model, c("D=0", "lag(y_break50_xts, 1):D=0"), test="F", white.adjust = FALSE)
