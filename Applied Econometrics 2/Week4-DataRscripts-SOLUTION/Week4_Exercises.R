...
end_dates <- seq(2007.00, 2013.75, 0.25)   
...
for (i in 1:P) {
   ...
   GDPGrowth_estimating_sample <- GDPGrowth_xts[index(GDPGrowth_xts) <  EndDate_YearQuarter] # Now we use this to limit our estimating sample
   ##### COMPLETE THIS MODEL below BY ADDING THE MISSING TERM from and AR2!
   AR2_lm <- lm(GDPGrowth_estimating_sample ~ "ADD REGRESSORS FOR AN AR2 MODEL of GDPGrowth_estimating_sample HERE") # Run an AR2 on the restricted estimating sample
   SER[i] <- summary(AR2_lm)$sigmaag(GDPGrowth_estimating_sample)
   ...
   # compute forecast
   ### COMPLETE TIHS FORECAST CODE BY FILLING IN THE ESTIMATED COEFFICIENTS WHERE NECESSARY
   ### YOU CAN FIND THESE COEFFICIENTS ABOVE
   pseudo_forecast <- ("INSERT COEFFICIENT HERE" + ("INSERT COEFFICIENT HERE" %*% GDPGrowth_xts[EndDate_YearQuarter - 0.25]) 
                       + ("INSERT COEFFICIENT HERE" %*% GDPGrowth_xts[EndDate_YearQuarter - 0.50]))
...
### Here we convert our saved values into xts objects for plotting below.
...
plot(as.zoo("REPLACE WITH xts OBJECT HERE"), ### FILL IN THIS BRACKET WITH THE _xts OBJECT FOR TRUE OUTCOMES
     col = "purple",
     lwd = 4,
     ylab = "Percent",
     main = "AR2: Pseudo-Out-Of-Sample Forecasts of UK GDP Growth")
# add the series of pseudo-out-of-sample forecasts
lines(as.zoo("REPLACE WITH xts OBJECT HERE"), ### FILL IN THIS BRACKET WITH THE _xts OBJECT FOR YOUR FORECASTS
      lwd = 4,
      lty = 2)
